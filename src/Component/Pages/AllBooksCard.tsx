...
//                 <Card className="w-full h-[500px] max-w-md bg-[#dac2c253] rounded-xl shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden">
...
const AllBooksCard = () => {
  ...
  useEffect(() => {
    fetch("/data.json") // Make sure the file is accessible from the public folder
      .then((res) => res.json())
      .then((data) => setData(data));
  }, []);

  const totalPages = Math.ceil(datas.length / booksPerPage);
  const indexOfLastBook = currentPage * booksPerPage;
  const indexOfFirstBook = indexOfLastBook - booksPerPage;
  const currentBooks = datas.slice(indexOfFirstBook, indexOfLastBook);
  ...
  return (
    <div className="w-11/12 mx-auto py-10 mt-20">
      <div className="text-center">
        <p className="text-sm font-bold hover:bg-purple-600 hover:text-black transition duration-500">
          All Book
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {currentBooks.map((data, index) => {
          const { img, title, author, genre, description, available, price } =
            data;
